...
for i = 1:step:40-step
	i
	% if i == 10
	% 	break
	% % end
	% s=xml2struct(strcat(Pcd_path,xml_files{i+step}));
	...
	if merging_strategy==1
		...
		% Find camera pose
		[result, R, t,RMS] = ICP(transpose(cloud_point_source),transpose(cloud_point_target),method,number_of_points);
	...
	elseif merging_strategy==2
		...
		% Find camera pose
		[result, R, t,RMS] = ICP(pcd_merged,transpose(cloud_point_target),method,number_of_points);
